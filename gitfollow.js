(function(){
  ...
  window.gitfollow = {
    ...
    // if you wish the text to be in a different language, here's where you translate it
    text: '% followers', 
    ...
    run: function(){
      this.elements = getByClass(this.className);
      if (!this.elements.length){
        if (window.console) console.log('No elements found to use with gitfollow.');
        return false;
      }      
      var match = this.elements[0].href.match(/com\/([^\s\/]+)/);
      if (!match || !match[1]){
      ...
      if (window.sessionStorage && sessionStorage['_gitfollow.' + this.username] != null){
        this.apply(sessionStorage['_gitfollow.' + this.username]);
      } else {
        injectScript('http://github.com/api/v2/json/user/show/'+ encodeURIComponent(this.username) +'?callback=' + encodeURIComponent('gitfollow.callback'));
      }      
    },
    
    callback: function(object){
      if (object && object.user){
        if (window.sessionStorage) sessionStorage['_gitfollow.' + this.username] = object.user['followers_count'];
        this.apply(object.user['followers_count']);
      }      
    },
    
    apply: function(count){      
      applyCss(this.styles.join(''));
      var text = '<img src="'+ this.logo +'" /> ' + this.text.replace(/%/, count);
  ...
  getByClass = function(){
    if (document.getElementsByClassName){
  	  return document.getElementsByClassName('gitfollow');
  	} else {
  		var links = document.getElementsByTagName('a'), found = [];
  		for (var i = 0, l = links.length; i < l; i++){
  			if ((' ' + this + ' ').indexOf(' ' + links[i].className + ' ') > -1) found.push(links[i]);  			
  		}
  		return found;
  	}
  },
